...
with st.sidebar:
    ...
    st.markdown("- Go to https://github.com/lastmile-ai/mcp-agent")
    ...
    st.markdown("- Navigate to https://github.com/lastmile-ai/mcp-agent, scroll down, and report details")
...
async def run_mcp_agent(message):
    ...
    # mcp_agent.config.yaml (base_url, default_model). OpenRouter and other
    # OpenAI-compatible servers are supported via the same `openai:` section.
    if not os.getenv("OPENAI_API_KEY") and not os.path.exists(
        os.path.join(os.path.dirname(__file__), "mcp_agent.secrets.yaml")
    ):
        return (
            "Error: no LLM credentials found. Add your OpenRouter API key to "
            "mcp_agent.secrets.yaml, or set the OpenAI-compatible "
...
if 'result' not in locals():
    st.markdown(
        ...
        <ol>
            <li>Enter your OpenRouter API key in your mcp_agent.secrets.yaml file</li>
...
# Footer
st.markdown("---")
st.write("Built with Streamlit, Playwright, and [MCP-Agent](https://www.github.com/lastmile-ai/mcp-agent) Framework for LLM-powered agents.")
